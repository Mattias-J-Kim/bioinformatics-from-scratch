...
- **pipelines/** — Workflows combining the modules above into an end-to-end
  analysis. `gc_window_pipeline.py`: GC content profile across a genome using
  100 bp non-overlapping windows (`fasta_parser` + `gc_content`).
...
```
python pipelines/gc_window_pipeline.py
```
...
documented in
[algorithms-practice](https://github.com/Mattias-J-Kim/algorithms-practice).
...
## Notes
- **reverse_complement/** contains both a recursive and an iterative implementation. The recursive version is the original exercise and is kept for that reason, but it fails with RecursionError past roughly 1,000 bp, so the iterative version is used for real sequences. six_frame_translation/ keeps its own copy of the iterative implementation rather than importing across module folders, since path manipulation is avoided outside of pipelines/.
...
- **de_bruijn_assembly/** returns a sequence that differs from the source string
  ...
  by string equality.
- **pipelines/gc_window_pipeline.py** discards the final partial window (86 bp of
...
`pipelines/gc_window_pipeline.py` was supplied by the assistant after I could not
